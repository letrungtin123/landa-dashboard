/**
 * MultiQuizEditor — Editor form cho Quiz Nhiều đáp án + Media
 *
 * Layout & CSS clone CHÍNH XÁC từ ProblemEditor (choiceresponse variant).
 *
...
import type { QuizChoice, PendingFile } from './SingleQuizEditor';

interface MultiQuizEditorProps {
  displayName: string;
...
export default function MultiQuizEditor({
  ...
  const handleUpdateChoice = (id: string, updates: Partial<QuizChoice>) => {
    // Multi quiz: toggle correct trực tiếp, cho phép nhiều đáp án đúng
  ...
  // ── Tổng hợp danh sách ảnh: saved + pending ──
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          ...
          {/* Đáp án — clone chính xác từ ProblemEditor (choiceresponse variant) */}
          <div className="space-y-3">
            <div>
              <h3 className="text-sm font-bold">Danh sách đáp án</h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                Nhập các đáp án bên dưới và đánh dấu vào các đáp án đúng.
              </p>
            </div>
                ...
                <div key={choice.id} className="flex gap-4 items-start group">
                  <div className="pt-[14px]">
                    <input
                      ...
                      onChange={e => handleUpdateChoice(choice.id, { correct: e.target.checked })}
